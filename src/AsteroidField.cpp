...
AsteroidField::~AsteroidField(){
	for (std::vector<Asteroid*>::iterator it = asteroidIndex.begin(); it != asteroidIndex.end(); it++)
		delete *it;
	deleteSubtreeStructure(root);
}


Asteroid* 
AsteroidField::getAsteroid(int id){
	return asteroidIndex[id];
}

int 
AsteroidField::getLength(){
	return asteroidIndex.size();
}

Asteroid* 
AsteroidField::addAsteroid(osg::Vec3f pos, float radius){
	Asteroid *a = new Asteroid(pos, radius);
	asteroidIndex.push_back(a);
	if (root == NULL)
		root = new Node(a);
	else
		root->addAsteroid(a);
...
AsteroidField::getAsteroidsInBlock(osg::Vec3f center, osg::Vec3f width){
	...
	treeSearchAsteroids(root, &boxLowCorner, &boxHighCorner, &result);
	return result;
}
